...
interface BusinessState {
  ...
  // Entry management
  addEntry: (entry: Omit<BookEntry, 'id' | 'createdAt' | 'userId'>, context?: { bookName?: string, currentBalance?: number, silent?: boolean }) => Promise<void>;
  updateEntry: (entryId: string, updates: Partial<Omit<BookEntry, 'id' | 'businessId' | 'bookId' | 'userId' | 'createdAt'>>, context?: { bookName?: string, currentBalance?: number, silent?: boolean }) => Promise<void>;
  deleteEntry: (entryId: string, context?: { bookName?: string, silent?: boolean }) => Promise<void>;
  transferEntry: (entry: BookEntry, targetBookId: string, options?: { silent?: boolean }) => Promise<void>;
...
export const [BusinessProvider, useBusiness] = createContextHook((): BusinessState => {
  ...
  const addEntry = useCallback(async (
    entryData: Omit<BookEntry, 'id' | 'createdAt' | 'userId'>, 
    context?: { bookName?: string, currentBalance?: number, silent?: boolean }
  ) => {
    if (!user || !currentBusiness || !db) return;
    ...
    try {
      ...
      if (entryData.type === 'cash_in') {
        updatePayload.totalCashIn = increment(amount);
      } else {
        updatePayload.totalCashOut = increment(amount);
      }
  ...
  const updateEntry = useCallback(async (
    entryId: string, 
    updates: Partial<Omit<BookEntry, 'id' | 'businessId' | 'bookId' | 'userId' | 'createdAt'>>, 
    context?: { bookName?: string, currentBalance?: number, silent?: boolean }
  ) => {
    if (!user || !currentBusiness || !db) return;
  ...
  const deleteEntry = useCallback(async (
    entryId: string, 
    context?: { bookName?: string, silent?: boolean }
  ) => {
    if (!user || !currentBusiness || !db) return;
  ...
  const sendBulkNotification = useCallback(async (options: {
    title: string;
    message: string;
      ...
      bulkMembersToNotify.forEach((memberId: string) => {
        ...
        batch.set(notifRef, {
          ...
          metadata: {
            ...
            businessId: currentBusiness.id,
            businessName: currentBusiness.name,
            sentBy: user.displayName || user.name || user.email
          }
        });
      });
      await batch.commit();
